...
+(NSDictionary *)dictionaryForKey:(NSString *)aKey
{
    ...
    {
        NSUbiquitousKeyValueStore* cloud = [NSUbiquitousKeyValueStore defaultStore];
        retv = [cloud dictionaryForKey:aKey];
        if (!retv)
        {
            retv = [[NSUserDefaults standardUserDefaults] dictionaryForKey:aKey];
            [cloud setDictionary:retv forKey:aKey];
        }
    }
    else {
        retv = [[NSUserDefaults standardUserDefaults] dictionaryForKey:aKey];
...
+(void)setBool:(BOOL)aBool forKey:(NSString*)aKey {
    [SDCloudUserDefaults setObject:@(aBool) forKey:aKey];
}
    ...
    if(kv)
        [[NSNotificationCenter defaultCenter] addObserverForName:@"NSUbiquitousKeyValueStoreDidChangeExternallyNotification"
                                                      ...
                                                      usingBlock:^(NSNotification* notification) {
                                                          ...
                                                          NSUbiquitousKeyValueStore* cloud = [NSUbiquitousKeyValueStore defaultStore];
                                                          NSDictionary* changedKeys = (notification.userInfo)[@"NSUbiquitousKeyValueStoreChangedKeysKey"];
