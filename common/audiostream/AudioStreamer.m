...
{	
	...
	switch (inPropertyID) {
		case kAudioFileStreamProperty_ReadyToProducePackets :
		{
			...
			if (!err) {
				DLog(@"formatListSize %u\n", (unsigned int)formatListSize);
				...
				for (int x = 0; x < formatListSize; x += sizeof(AudioFormatListItem)){
					...
					DLog(@"rate: %lf  Format:%u%u%u%u  FramesPerPacket:%u  bytesPerFrame:%u ChannelsperFrame:%u\r\n",
						  pasbd->mSampleRate, (unsigned int)(pasbd->mFormatID>>24)&255, (unsigned int)(pasbd->mFormatID>>16)&255, (unsigned int)(pasbd->mFormatID>>8)&255, (unsigned int)(pasbd->mFormatID)&255, (unsigned int)pasbd->mFramesPerPacket, (unsigned int)pasbd->mBytesPerFrame, (unsigned int)pasbd->mChannelsPerFrame);
...
void MyAudioQueueIsRunningCallback(void *inUserData, AudioQueueRef inAQ, AudioQueuePropertyID inID)
{
	...
	else
	{
        ...
        [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:kStreamConnected object:nil]];
...
void MyAudioSessionInterruptionListener(void *inClientData, UInt32 inInterruptionState)
{
	...
	if (inInterruptionState == kAudioSessionBeginInterruption) {
		[radio stop];
		DLog(@"kAudioSessionBeginInterruption");
	}
	else if (inInterruptionState == kAudioSessionEndInterruption) {
		//[radio start]; // this doesn't work - radio is gone
		DLog(@"kAudioSessionEndInterruption");
	}
}
#endif
...
)
{
	...
	{
		//DLog(@"kCFStreamEventHasBytesAvailable");
		if (myData->failed || myData->finished)
		{
			return;
		}
		...
		if (length > 0)
		{
			...
			if (myData->metaDataInterval == 0)
			{
				...
				UInt32 statusCode = (UInt32)CFHTTPMessageGetResponseStatusCode(myResponse);
				...
				if (statusCode == 200)		// "OK" (this is true even for ICY)
				{
					...
					else
					{
						...
						if (contentType) 
						{
							...
							{
								DLog(@"Stream Content-Type: %@", contentType);
								myData.streamContentType = contentType;
								// if this is an application/ogg stream, audio should be aborted.
								if ([myData.streamContentType caseInsensitiveCompare:@"application/ogg"] == NSOrderedSame)
								{
                                    NSLog(@"OGG detected. Abort streaming");
                                    [myData stop];
									[myData audioStreamerError];
                                    return;
								}                                
			...
			if (myData.foundIcyStart && !myData.foundIcyEnd)
			{
					...
					while (YES)
					{
						...
						{		
							...
							DLog(@"Header Line: %@. Length: %d", line, (int)[line length]);
							...
							if ([lineItems count] > 1)
							{
								if ([[lineItems objectAtIndex:0] caseInsensitiveCompare:@"icy-metaint"] == NSOrderedSame)
								{
									...
									DLog(@"ICY MetaInt: %d", myData->metaDataInterval);
								}
								...
								{
									DLog(@"ICY Stream Content-Type: %@", [lineItems objectAtIndex:1]);
                                ...
                                // TODO: Implement additional headers
								if ([[lineItems objectAtIndex:0] caseInsensitiveCompare:@"icy-name"] == NSOrderedSame)
                                {
                                    myData.streamRadioName = [[line substringFromIndex:9] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                                    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:kStreamGotRadioname object:nil]];

                                }
								if ([[lineItems objectAtIndex:0] caseInsensitiveCompare:@"icy-genre"] == NSOrderedSame)
                                {
                                    myData.streamGenre = [[line substringFromIndex:10] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                                    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:kStreamGotGenre object:nil]];
                                }
								if ([[lineItems objectAtIndex:0] caseInsensitiveCompare:@"icy-url"] == NSOrderedSame)
                                {
                                    myData.streamRadioUrl = [[line substringFromIndex:8] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                                    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:kStreamGotRadioUrl object:nil]];
                                }
			...
			if (myData.parsedHeaders)
			{
				...
				for (int i=streamStart; i < length; i++)
				{
					...
					if (myData->metaDataBytesRemaining > 0)
					{
                        ...
                        [myData.metaDataData appendBytes:&(bytes[i]) length:1];
						...
						if (myData->metaDataBytesRemaining == 0)
						{
                            ...
                            if(myData.metaDataString)
                                [myData.metaDataString release];
                            myData.metaDataString = [[NSString alloc] initWithData:myData.metaDataData encoding:NSUTF8StringEncoding];
							DLog(@"MetaData: %@.", myData.metaDataString);
					...
					if (myData->metaDataInterval > 0 && myData->dataBytesRead == myData->metaDataInterval)
					{
                        ...
                        if(myData.metaDataData)
                            [myData.metaDataData release];
                         myData.metaDataData = [[NSMutableData alloc] initWithCapacity:myData->metaDataBytesRemaining];
						...
						{
							DLog(@"Found interval. Meta Length: %d", myData->metaDataBytesRemaining);
						}
			...
			if (myData->discontinuous)
			{
				...
				else if (myData->metaDataInterval == 0)	// make sure this isn't a stream with metadata
				{
					...
					OSStatus err = AudioFileStreamParseBytes(myData->audioFileStream, (UInt32)length, bytes, kAudioFileStreamParseFlag_Discontinuity);
					if (err) { PRINTERROR("AudioFileStreamParseBytes"); myData->failed = true;}					
				}
			}
			else
			{
				...
				else if (myData->metaDataInterval == 0)	// make sure this isn't a stream with metadata
				{
					...
					OSStatus err = AudioFileStreamParseBytes(myData->audioFileStream, (UInt32)length, bytes, 0);
					if (err) { PRINTERROR("AudioFileStreamParseBytes"); myData->failed = true; }
				}
			} // end discontinuous
...
@synthesize streamRadioName;
@synthesize streamGenre;
@synthesize bitRate;
@synthesize streamRadioUrl;
@synthesize metaDataData;
...
//					alternateButton:nil
//					otherButton:nil
//					informativeTextWithFormat:NSLocalizedString(@"Attempt to play streaming audio failed.", @"")];
//			[alert
...
// Significant event to be notified.
- (void)updateMetaData:(NSString *)metaData
{
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:kStreamHasMetadata object:nil]];
}

- (void)audioStreamerError
{
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:kStreamIsInError object:nil]];
}

- (void)redirectStreamError:(NSURL*)redirectURL;
{
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:kStreamIsRedirected object:nil]];
}

- (void)updateBitrate:(uint32_t)br 
{
    [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:kStreamHasBitrate object:nil]];
}
...
- (void)restartAudioQueue
{
	...
	{
		DLog(@"Audio Type Hint: %@", streamContentType);
...
- (void)stop
{
	...
	if (stream)
	{
		...
		DLog(@"AudioStreamer closed stream.");
		if (finished)
		{
			return;
		}
		...
		if (started)
		{
			...
			DLog(@"AudioStreamer stopped queue.");
		}
		else
		{
			self.isPlaying = true;
