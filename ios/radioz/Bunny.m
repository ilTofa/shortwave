...
-(void)errorInAnswer:(NSString *)dataReceived
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Error", @"") message:self.lastMessage delegate:nil cancelButtonTitle:NSLocalizedString(@"Cancel", @"") otherButtonTitles: nil];
        [alert show];
    });
}
...
- (BOOL) sendRequestAndParse:(NSString *)urlString withVisibleErrors:(BOOL)errorsToTheUser
{
    ...
    if(response.statusCode != 200)
    {
        ...
        {
            dispatch_async(dispatch_get_main_queue(), ^{
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Error", @"") message:message delegate:nil cancelButtonTitle:NSLocalizedString(@"Cancel", @"") otherButtonTitles: nil];
                [alert show];
            });
        }
        else
...
-(void)keepaliveKarotzSession:(NSTimer *)timer
{
    ...
    const CGFloat *currentColors = CGColorGetComponents([UIColor colorWithHue:self.ledColor saturation:1.0 brightness:1.0 alpha:1.0].CGColor);
    ...
    if(self.ledColor >= 1.0)
        self.ledColor = 0.0;
    NSString *keepaliveUrl = [NSString stringWithFormat:@"http://api.karotz.com/api/karotz/led?action=light&color=%06X&interactiveid=%@", thisColor, self.interactiveId];
    DLog(@"About to send keepalive for hue %.2f, RGB #%06X", self.ledColor, thisColor);
